...
interface OtSettingsRow {
  user_id: string;
  data: { notifyEnabled?: boolean; defaultTimeOut?: string } | null;
...
Deno.serve(async (req) => {
  ...
  await Promise.allSettled(rows.map(async (row) => {
    ...
    const defaultTimeOut = row.data?.defaultTimeOut;
    if (!defaultTimeOut) return; // ยังไม่ได้ตั้งเวลาเลิกงานปกติ

    const targetMinutes = parseTimeToMinutes(defaultTimeOut);
    ...
    // มีการบันทึกเวลาทำงานของวันนี้แล้ว - ไม่ต้องเตือน
